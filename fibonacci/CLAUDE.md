...
- `LandingPage` collects a trimmed, non-empty name plus a `User`/`Facilitator` role radio (defaults to `Facilitator` — intentional, locked in by a `LandingPage.test.tsx` case; don't "fix" this) and calls `onSubmit(name, role)` (Enter key or button); `App.tsx` wires that to either `createRoom` or `joinRoom` depending on whether a room ID is present in the URL. `LandingPage` itself has no concept of rooms.
